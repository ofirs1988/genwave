...
if (!defined('LITELLM_API_URL')) {
    define('LITELLM_API_URL', 'https://api.genwave.ai');
}
 ...
 */
$gen_wave_encryption_key = get_option('aiaw_encryption_key');
...
function genwave_plugin_activation() {
    ...
    $existing_key = get_option('aiaw_encryption_key');

    if (!$existing_key) {
        // Save default key to wp_options on first activation
        $default_key = 'ATW1kctl7zkJDLC7IRC8JDfPBrgREiLu';
        update_option('aiaw_encryption_key', $default_key, false); // false = don't autoload
