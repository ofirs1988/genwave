...
class AjaxManager
{
    ...
    public function __construct()
    {
        ...
        add_action('wp_ajax_verify_login', [VerifyLoginController::class , 'verifyLogin']);
        add_action('wp_ajax_nopriv_verify_login', [VerifyLoginController::class , 'verifyLogin']);
        ...
        add_action('wp_ajax_get_all_products', [$this->productHandler, 'getAllProducts']);
        add_action('wp_ajax_get_all_posts', [$this->productHandler, 'getAllPosts']);
        add_action('wp_ajax_get_post_data', [$this->productHandler, 'handle_get_post_data']);
        add_action('wp_ajax_nopriv_get_post_data', [$this->productHandler, 'handle_get_post_data']);

        add_action('wp_ajax_refresh_tokens_action', [$this, 'handle_refresh_tokens']);
        add_action('wp_ajax_disconnect_account_action', [$this, 'handle_disconnect_account']);

        // Polling handlers - delegated to PollingHandler
        add_action('wp_ajax_ai_polling_results', [$this->pollingHandler, 'handle_ai_polling_results']);
        add_action('wp_ajax_nopriv_ai_polling_results', [$this->pollingHandler, 'handle_ai_polling_results']);
        add_action('wp_ajax_ai_poll_proxy', [$this->pollingHandler, 'handle_ai_poll_proxy']);
        add_action('wp_ajax_nopriv_ai_poll_proxy', [$this->pollingHandler, 'handle_ai_poll_proxy']);
        add_action('wp_ajax_update_token_balance', [$this->pollingHandler, 'handle_update_token_balance']);
        add_action('wp_ajax_nopriv_update_token_balance', [$this->pollingHandler, 'handle_update_token_balance']);
    ...
    public function handle_refresh_tokens()
    {
        ...
        $tokens = (new TokensController())->RefreshTokens();
        Config::set('tokens', $tokens);

        if (isset($tokens['error']) && $tokens['error'] === true) {
            Config::delete('uidd');
            Config::delete('token');
            ...
            Config::delete('plan');
            wp_send_json_error([
                'auth' => $tokens['auth'] ?? false,
                'url' => $tokens['url'] ?? '',
                'btn_text' => $tokens['btn_text'] ?? 'Learn more',
                'message' => $tokens['message'] ?? 'An error occurred.'
            ]);
        } else {
    ...
    public function handleAdminAction()
    {
        ...
        update_option('my_admin_option', $data);
    ...
    public function handleResponse($response) {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_var_dump -- Debug mode only
            var_dump($body);
        }
    ...
    public function handle_ai_polling_results()
    {
        if (defined('WP_DEBUG') && WP_DEBUG) {
            ...
            // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log,WordPress.PHP.DevelopmentFunctions.error_log_print_r,WordPress.Security.NonceVerification.Missing -- Debug mode only
    ...
    public function handle_ai_poll_proxy() {
        ...
        // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Action check before nonce verification
        $action = isset($_POST['action']) ? sanitize_text_field(wp_unslash($_POST['action'])) : '';
        if ($action !== 'ai_poll_proxy') {
            if (defined('WP_DEBUG') && WP_DEBUG) {
                // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Debug mode only
                error_log('AI Poll Proxy (AjaxManager): Wrong action received: ' . $action);
            }
    ...
    public function handle_get_post_data() {
        try {
            ...
            if (isset($_POST['nonce']) && !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'ai_awesome_nonce')) {
                if (defined('WP_DEBUG') && WP_DEBUG) {
                    // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Debug mode only
                    error_log('Get Post Data AJAX: Invalid nonce');
                }
                // Don't fail hard on nonce check since we need this to work
            }
    ...
    public function handle_genwave_generate_single() {
        ...
        try {
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) {
                ...
                // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log,WordPress.PHP.DevelopmentFunctions.error_log_print_r -- Debug mode only
                error_log('Gen Wave Generate Single: POST data: ' . print_r($_POST, true));
            }
