...
class ApiManager {
    ...
    public function postToLiteLLM($endpoint, $data = [], $token = null, $uidd = null) {
        ...
        $url = rtrim(LITELLM_API_URL, '/') . '/api/v1/' . ltrim($endpoint, '/');
        ...
        // Only disable SSL verification for localhost/development
        $is_localhost = (defined('LITELLM_API_URL') &&
                        (strpos(LITELLM_API_URL, 'localhost') !== false ||
                         strpos(LITELLM_API_URL, '127.0.0.1') !== false ||
                         strpos(LITELLM_API_URL, '.local') !== false));
        ...
        // Handle non-200 status codes
        if ($response_code !== 200) {
            $error_message = "LiteLLM API returned status code: $response_code";

            // Try to parse error details from response
            $decoded = json_decode($body, true);
            if ($decoded && isset($decoded['detail'])) {
                if (is_array($decoded['detail'])) {
                    $error_message .= ' - ' . json_encode($decoded['detail']);
                } else {
    ...
    public function checkLiteLLMJobStatus($requestId, $token = null, $uidd = null) {
        ...
        $url = rtrim(LITELLM_API_URL, '/') . '/api/v1/job/' . $requestId;
    ...
    public function getLiteLLMJobResults($requestId, $token = null, $uidd = null) {
        ...
        $url = rtrim(LITELLM_API_URL, '/') . '/api/v1/job/' . $requestId . '/results';
     ...
     */
    public function callLiteLLMStreaming($posts_or_request_data, $language = null, $generation_method = null, $length = null, $type = 'single', $async = false) {
        ...
        $request_data = [
            ...
            'provider' => 'openai',
            'model' => 'gpt-4o-mini',
            'selectedOptions' => [$generation_method ?? 'description'],
            'generateOptions' => [
                'length' => $length ?? 1000,
                'type' => $type ?? 'single'
            ]
    ...
    private function callLiteLLMStreamingInternal($request_data) {
        try {
            ...
            $litellm_url = defined('LITELLM_API_URL')
                ? LITELLM_API_URL . $endpoint
            ...
            $litellm_payload = [
                ...
                'instructions' => [
                    ...
                    'Language: ' . ($request_data['language'] ?? 'English'),
                    'Focus on: ' . implode(', ', $request_data['selectedOptions'] ?? ['description'])
                ],
                'selectedOptions' => $request_data['selectedOptions'] ?? ['description'],
            ...
            if ($response_code !== 200) {
                ...
                $error_message = 'LiteLLM API Error (Code: ' . $response_code . ')';
    ...
    private function getBusinessContext() {
        ...
        $settings = get_option('gen_wave_mybusiness_settings', []);
