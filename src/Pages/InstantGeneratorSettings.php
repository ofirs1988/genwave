...
class InstantGeneratorSettings
{
    public function __construct()
    {
        ...
        add_action('wp_ajax_ai_test_connection', [$this, 'test_api_connection']);
    }
    ...
    {
        register_setting('ai_generator_settings', 'ai_api_base_url', [
            'type' => 'string',
            'sanitize_callback' => 'sanitize_url',
            'default' => 'https://account.genwave.ai/api/v1'
        ]);

        register_setting('ai_generator_settings', 'ai_api_key', [
            'type' => 'string',
            'sanitize_callback' => 'sanitize_text_field',
            'default' => ''
        ]);

        register_setting('ai_generator_settings', 'ai_default_model', [
            'type' => 'string',
            'sanitize_callback' => 'sanitize_text_field',
            'default' => 'gpt-3.5-turbo'
        ]);

        register_setting('ai_generator_settings', 'ai_default_provider', [
            'type' => 'string',
            'sanitize_callback' => 'sanitize_text_field',
            'default' => 'OpenAI'
        ]);

        register_setting('ai_generator_settings', 'ai_rate_limit', [
            'type' => 'integer',
    ...
    public function render_settings_page()
    {
        ...
        if (isset($_GET['settings-updated'])) {
            add_settings_error(
                ...
                'ai_generator_message',
                'Settings saved successfully!',
                'updated'
            );
        }

        settings_errors('ai_generator_messages');
        ?>
        <div class="wrap">
            ...
            <form action="options.php" method="post">
                <?php
                settings_fields('ai_generator_settings');
                do_settings_sections('ai_generator_settings');
        ...
        jQuery(document).ready(function($) {
            $('#test-connection-btn').on('click', function() {
                ...
                $.post(ajaxurl, {
                    ...
                    nonce: '<?php echo esc_js(wp_create_nonce('ai_test_connection')); ?>'
                }, function(response) {
    ...
    {
        if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'ai_test_connection')) {
    ...
    {
        $value = get_option('ai_api_base_url', 'https://account.genwave.ai/api/v1');
        printf(
            '<input type="url" id="ai_api_base_url" name="ai_api_base_url" value="%s" class="regular-text" placeholder="https://your-domain.com/api/v1" />',
            esc_attr($value)
        );
    ...
    {
        $value = get_option('ai_api_key', '');
        printf(
            '<input type="password" id="ai_api_key" name="ai_api_key" value="%s" class="regular-text" placeholder="your-api-key-here" />',
            esc_attr($value)
        );
    ...
    {
        $value = get_option('ai_default_provider', 'OpenAI');
        $providers = ['OpenAI', 'Anthropic', 'Google'];
        
        echo '<select id="ai_default_provider" name="ai_default_provider">';
    ...
    {
        $value = get_option('ai_default_model', 'gpt-3.5-turbo');
        $models = [
        ...
        echo '<select id="ai_default_model" name="ai_default_model">';
    ...
    {
        $value = get_option('ai_rate_limit', 10);
        printf(
            '<input type="number" id="ai_rate_limit" name="ai_rate_limit" value="%s" min="1" max="100" class="small-text" />',
            esc_attr($value)
        );
